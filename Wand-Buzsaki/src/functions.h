...
extern unsigned netSeed;

dim2 read_matrix(std::string, int);
bool fileExists(const std::string &);
void display_timing(double wtime, double cptime);
double get_wall_time();
double get_cpu_time();
dim1 normal_dist(int, double, double);
dim1 set_initial_conditions(int);
...
double measureAverageISI(const dim2 &);
